...
<section class="article-hero-modern">
    ...
    <div class="container hero-content-modern">
        <div class="article-meta">
            <a href="{{ route('front.explorer') }}" class="meta-chip">
                <i class="fas fa-compass"></i>
                Explorer
            </a>
            @if($contenu->region)
            <a href="{{ route('front.regions', ['id' => $contenu->region->id_region]) }}" class="meta-chip">
        ...
        <div class="article-meta" data-aos="fade-up" data-aos-delay="200">
            <span class="meta-chip">
                ...
                {{ number_format($views) }} vues
            </span>
            <span class="meta-chip">
                <i class="far fa-calendar"></i>
                {{ $contenu->created_at->translatedFormat('d F Y') }}
            </span>
...
<div class="floating-action-bar" id="floatingActions">
    ...
    <button class="fab-btn" onclick="toggleLikeModern()" id="likeBtnModern" title="J'aime">
        ...
        <span class="fab-btn-badge" id="likeCountBadge">{{ $likes }}</span>
    </button>
    ...
    <button class="fab-btn" onclick="scrollToSection('comments')" title="Commentaires">
        ...
        <span class="fab-btn-badge" id="commentCountBadge">{{ $commentsCount }}</span>
    </button>
...
<div class="container">
    <div class="author-card-glass" data-aos="fade-up">
        <div class="d-flex align-items-center">
            <div class="author-avatar-circle" style="background: {{ $authorColor }};">
                @if($hasAuthorPhoto && $authorPhoto)
                    <img src="{{ $authorPhoto }}" alt="{{ $authorName }}"
                         onerror="this.style.display='none'; this.parentElement.innerHTML='{{ $authorInitials }}';">
                @else
                    {{ $authorInitials }}
                @endif
            </div>
            ...
            <div class="flex-grow-1">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h3 class="fw-bold mb-1">{{ $authorName }}</h3>
                        <p class="text-muted mb-2">
                            {{ $author->role->nom_role ?? 'Contributeur Culturel' }}
                            @if($author->followers_count)
                            <span class="ms-2">
                                <i class="fas fa-users me-1"></i>
                                {{ $author->followers_count }} abonnés
                            </span>
                            @endif
                        </p>
                        @if($author->bio)
                        <p class="mb-0 text-muted">{{ Str::limit($author->bio, 120) }}</p>
                        @endif
                    </div>
...
<main class="content-modern">
    ...
    <div class="stats-modern" data-aos="fade-up">
        <div class="stat-card">
            <div class="stat-number" id="viewsStat">{{ number_format($views) }}</div>
            <div class="stat-label">Vues</div>
        </div>
        <div class="stat-card">
            <div class="stat-number" id="likesStat">{{ number_format($likes) }}</div>
            <div class="stat-label">Likes</div>
        </div>
        <div class="stat-card">
            <div class="stat-number" id="commentsStat">{{ number_format($commentsCount) }}</div>
    ...
    <div class="tags-modern" data-aos="fade-up">
        @foreach($tags as $tag)
        <a href="{{ route('front.explorer', ['search' => $tag['text']]) }}" class="tag-modern">
            ...
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    <img src="{{ $image }}"
                         ...
                         onerror="this.src='{{ ImageHelper::defaultContent() }}'">
                </div>
                @endforeach
            </div>
    ...
    <div class="d-flex justify-content-center gap-3 my-5" data-aos="fade-up">
        <button class="btn btn-lg btn-primary px-4" onclick="toggleLikeModern()">
            ...
            <span class="badge bg-white text-primary ms-2" id="likeBadge">{{ $likes }}</span>
        </button>
        <button class="btn btn-lg btn-outline-primary px-4" onclick="scrollToSection('comments')">
            <i class="far fa-comment me-2"></i>
            Commenter
            <span class="badge bg-primary text-white ms-2">{{ $commentsCount }}</span>
        </button>
    ...
    <section class="comments-modern" id="comments" data-aos="fade-up">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            @else
            <a href="{{ route('front.connexion') }}?redirect={{ url()->current() }}" class="btn btn-primary">
        ...
        <div class="comment-form-modern mb-4" id="commentFormModern" style="display: none;">
            <div class="d-flex gap-3">
                ...
                @endphp
                <div class="comment-avatar" style="background: {{ $userAvatar['color'] }};">
                    @if($userAvatar['has_photo'] && $userAvatar['photo_url'])
                        <img src="{{ $userAvatar['photo_url'] }}" alt="{{ $currentUser->name }}">
                    @else
...
<div id="commentsListModern">
    ...
    @if($comments->count() > 0)
        ...
        <div class="comment-card-modern" id="comment-{{ $comment->id }}">
            <div class="d-flex gap-3">
                @php
                    $commentAuthor = $comment->auteur ?? null;
                    $commentAvatar = ImageHelper::getUserAvatarInfo($commentAuthor);
                @endphp
                <div class="comment-avatar" style="background: {{ $commentAvatar['color'] }};">
                    @if($commentAvatar['has_photo'] && $commentAvatar['photo_url'])
                        <img src="{{ $commentAvatar['photo_url'] }}" alt="{{ $commentAvatar['name'] }}">
                    @else
                        <div class="w-100 h-100 d-flex align-items-center justify-content-center text-white fw-bold">
                ...
                <div class="flex-grow-1">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div>
                            <h6 class="fw-bold mb-0">{{ $commentAvatar['name'] }}</h6>
                    ...
                    <div class="comment-actions">
                        <button class="comment-action-btn" onclick="likeCommentModern({{ $comment->id }})" id="commentLikeBtn-{{ $comment->id }}">
                            <i class="far fa-heart"></i>
                            <span id="commentLikeCount-{{ $comment->id }}">{{ $comment->likes_count ?? 0 }}</span>
                        </button>
    ...
    <section class="related-slider" data-aos="fade-up">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <a href="{{ route('front.explorer') }}" class="btn btn-link text-primary">
                Voir plus
        ...
        <div class="row g-4">
            @foreach($relatedContent->take(3) as $related)
            <div class="col-md-4">
                <a href="{{ route('front.contenu', ['id' => $related->id_contenu]) }}" class="related-card-modern">
                    <div class="related-image-container">
                        <img src="{{ ImageHelper::getContentImage($related) }}"
                             alt="{{ $related->titre }}"
                             class="related-image-overlay">
                        <span class="related-badge">
                            {{ $related->typeContenu->nom_contenu ?? 'Article' }}
                        </span>
                    </div>
                    <div class="p-3 flex-grow-1">
                        <h5 class="fw-bold mb-2" style="font-size: 1.1rem;">
                            {{ Str::limit($related->titre, 50) }}
                        </h5>
                        <p class="text-muted small mb-3">
                            {{ Str::limit(strip_tags($related->texte ?? ''), 80) }}
                        </p>
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <div class="me-2">
                                    <div class="rounded-circle d-flex align-items-center justify-content-center"
                                         style="width: 24px; height: 24px; background: {{ $relatedAvatar['color'] }}; color: white; font-size: 0.7rem;">
                                        {{ $relatedAvatar['initials'] }}
                                    </div>
                                </div>
                                <span class="small">{{ $relatedAvatar['name'] }}</span>
                            </div>
                            <small class="text-muted">
                                <i class="far fa-clock me-1"></i>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.toggleLikeModern = async function() {
        if (!{{ auth()->check() ? 'true' : 'false' }}) {
            ...
            setTimeout(() => {
                window.location.href = '{{ route("front.connexion") }}?redirect=' + encodeURIComponent(window.location.href);
            }, 1500);
            return;
        }
        ...
        // Envoyer la requête
        try {
            const response = await fetch('{{ route("like.toggle", ["id" => $contenu->id_contenu]) }}', {
                method: 'POST',
                headers: {
    ...
    window.toggleBookmarkModern = async function() {
        ...
        // Envoyer la requête
        try {
            const response = await fetch('{{ route("favorite.toggle", ["id" => $contenu->id_contenu]) }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Accept': 'application/json'
    ...
    document.getElementById('followAuthorBtnModern')?.addEventListener('click', async function() {
        ...
        // Envoyer la requête
        try {
            const response = await fetch('{{ route("follow.toggle", ["authorId" => $author->id]) }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Accept': 'application/json'
    ...
    window.shareContentModern = function() {
        const shareData = {
            title: '{{ $contenu->titre }}',
            text: 'Découvrez ce contenu sur Bénin Culture : {{ Str::limit($contenu->titre, 100) }}',
    ...
    document.getElementById('commentFormElModern')?.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch('{{ route("comment.add", ["id" => $contenu->id_contenu]) }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Accept': 'application/json',
